...
OLD_FLOOR = ["floor-wall-logo-official.webp", "floor-wall-logo-safe.png", "floor-wall-icon.jpg"]
...
FORBIDDEN_BRAND = ["HERRAMIENTAS STRUCTURA", "ESTRUCTURA", "EINES STRUCTURA", "OUTILS STRUCTURA", "STRUMENTI STRUCTURA"]
...
def normalize_page(path: Path) -> bool:
    ...
    for name in OLD_STRUCTURA:
        text = text.replace(f"/assets/{name}", FINAL_LOGO).replace(f"assets/{name}", FINAL_LOGO.lstrip("/"))
    for name in OLD_FLOOR:
        text = text.replace(f"/assets/{name}", FINAL_FLOOR).replace(f"assets/{name}", FINAL_FLOOR.lstrip("/"))

    text = text.replace("/assets/road-click.webp", "/assets/road-click-safe.png").replace("assets/road-click.webp", "assets/road-click-safe.png")
    text = text.replace("/assets/buildings.webp", "/assets/buildings-safe.png").replace("assets/buildings.webp", "assets/buildings-safe.png")
    text = text.replace("/assets/pau-cols-founder.webp", "/assets/pau-cols-final.jpg").replace("assets/pau-cols-founder.webp", "assets/pau-cols-final.jpg")

    brand_rx = re.compile(r'<a\b[^>]*class=["\'][^"\']*\bbrand\b[^"\']*["\'][^>]*>.*?</a>', re.I | re.S)
    def fix_brand(m):
        ...
        if re.search(r'<span\b', block, re.I):
            ...
            block = re.sub(r'(</a>)', r'<span>STRUCTURA TOOLS</span>\1', block, count=1, flags=re.I)
        return block
    ...
    text = re.sub(r'(<h1\b[^>]*class=["\'][^"\']*\blanding-title\b[^"\']*["\'][^>]*>).*?(<span\b)', r'\1STRUCTURA TOOLS\2', text, count=1, flags=re.I | re.S)

    tile_rx = re.compile(r'<[^>]+class=["\'][^"\']*\bbrand-tile\b[^"\']*["\'][^>]*>.*?</(?:a|div)>', re.I | re.S)
    def fix_tile(m):
        block = replace_first_img_src(m.group(0), f"{FINAL_LOGO}?v={VERSION}")
        return re.sub(r'(<strong\b[^>]*>).*?(</strong>)', r'\1Structura Tools\2', block, count=1, flags=re.I | re.S)
    ...
    for old,new in TRANSLATED_BRAND.items(): text=text.replace(old,new)
...
def verify_page(path: Path) -> list[str]:
    ...
    for name in OLD_STRUCTURA+OLD_FLOOR:
        if name in text: errors.append(f"{path}: legacy/broken asset remains: {name}")
    for phrase in FORBIDDEN_BRAND:
        if phrase in text: errors.append(f"{path}: translated brand remains: {phrase}")

    brand_match=re.search(r'<a\b[^>]*class=["\'][^"\']*\bbrand\b[^"\']*["\'][^>]*>.*?</a>',text,re.I|re.S)
    if brand_match:
        block=brand_match.group(0)
        if "structura-logo-final.png" not in block: errors.append(f"{path}: header brand does not use definitive logo")
        plain=re.sub(r'<[^>]+>',' ',block)
        if not re.search(r'\bSTRUCTURA\s+TOOLS\b',plain,re.I): errors.append(f"{path}: header brand name is not Structura Tools")

    if "landing-title" in text:
        title=re.search(r'<h1\b[^>]*class=["\'][^"\']*\blanding-title\b[^"\']*["\'][^>]*>(.*?)</h1>',text,re.I|re.S)
        if title and not re.match(r'\s*STRUCTURA TOOLS\s*<span\b',title.group(1),re.I|re.S): errors.append(f"{path}: hero brand name is not Structura Tools")
        hero_logo=re.search(r'<img\b[^>]*class=["\'][^"\']*\blanding-hero-logo\b[^"\']*["\'][^>]*>',text,re.I|re.S)
        if hero_logo and "structura-logo-final.png" not in hero_logo.group(0): errors.append(f"{path}: hero does not use definitive logo")
    return errors
...
def main():
    ...
    errors=[]
    for p in pages: errors.extend(verify_page(p))
    print(f"Processed {len(pages)} HTML pages; changed {len(changed)}")
    for p in changed: print(f"  updated {p}")
    if errors:
